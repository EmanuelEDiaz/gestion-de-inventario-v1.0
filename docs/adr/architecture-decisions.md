...
## ADR-014: Leaflet → MapLibre GL JS

**Fecha**: 2026-06-05

**Contexto**: El stack de mapas original usaba Leaflet + react-leaflet + leaflet.vectorgrid + pmtiles v4.4.1. Este stack presentaba limitaciones para el objetivo offline-first:
...
**Decisión**: Migrar a MapLibre GL JS con PMTiles nativo.

**Criterio de retiro de Leaflet**: Eliminar componente Leaflet (`OfflineMap.tsx`, `CubaTileManager.ts`, `CubaGeoSearchAdapter.ts`) CUANDO:
...
**Estado actual**: Fase E completada (commit `2ee5604`). Leaflet aún presente como fallback durante transición. No hay fecha definida para retiro.
