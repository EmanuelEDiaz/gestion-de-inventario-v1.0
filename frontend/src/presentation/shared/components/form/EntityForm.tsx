...
export interface EntityFormField {
  name: string;
  label: string;
  type: 'text' | 'number' | 'date' | 'select' | 'radio-group';
  required?: boolean;
  placeholder?: string;
  maxLength?: number;
  step?: string;
...
interface EntityFormProps {
  ...
  error?: string | null;
  className?: string;
  /** Pre-fill empty fields in create mode. Ignored when isEditing. */
  initialValues?: Record<string, unknown>;
  /** Auto-save last submitted values to localStorage for pre-fill on next create.
   *  Use a unique key per form (e.g. "currency-create"). */
  storageKey?: string;
}
...
export function EntityForm({
  ...
  }, [onSubmit, storageKey, isEditing, values]);

  const defaultRender = useCallback(
    (field: EntityFormField) => {
      const filteredOptions = getFilteredOptions(field);
      const value = values[field.name] ?? '';
      const handleChange = (v: string) => onChange(field.name, v);

      return (
        <div key={field.name} className={cn(field.type === 'radio-group' ? 'space-y-2' : 'space-y-1', field.className)}>
          <LabelWithHint
    ...
    [getFilteredOptions, values, onChange],
  );
